...
export default function SelectLang({ t }: { t: NavAndLanguagesType["Languages"] }) {
  ...
  const languageOptions = useMemo(() => [
    { code: "en" as const, name: t.en || "English" },
    { code: "de" as const, name: t.de || "Deutsch" },
    { code: "fa" as const, name: t.fa || "Farsi" },
  ], [t.en, t.de, t.fa]);
  ...
  return (
    <div className="relative">
      <Button
        ...
        aria-haspopup="true"
      >
        <Languages className="w-5 h-5 text-foreground" aria-hidden="true" />
        <span className="text-xs md:text-base font-semibold uppercase tracking-wide text-foreground">
          {currentLocale}
        </span>
      </Button>
      ...
      {isMenuOpen && (
        <>
          ...
          <ul
            className="absolute right-0 mt-2 w-44 bg-popover border border-border rounded-lg shadow-lg dark:shadow-2xl dark:shadow-black/40 overflow-hidden animate-in fade-in-0 zoom-in-95 slide-in-from-top-2 duration-200 z-20"
            role="menu"
            aria-label="Language selection"
          >
            {languageOptions.map(({ code, name }) => (
              ...
              >
                <span className="flex-1">{name}</span>
                {code === currentLocale && (
                  <Check className="w-4 h-4 ml-2 text-accent-foreground flex-shrink-0" aria-hidden="true" />
                )}
              </li>
            ))}
          </ul>
